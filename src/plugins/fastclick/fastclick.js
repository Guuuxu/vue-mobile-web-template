...
function FastClick(layer, options) {
  ...
  // 修复滚动过程中可点击的补丁
  layer.isScrolling = false;
  layer.scrollTimeout = null;
  layer.addEventListener(
    'scroll',
    function () {
      layer.isScrolling = true;
      clearTimeout(layer.scrollTimeout);
      layer.scrollTimeout = setTimeout(function () {
        layer.isScrolling = false;
      }, 100);
    },
    true
...
FastClick.prototype.onTouchStart = function (event) {
  ...
  if (deviceIsIOS) {
    ...
    if (!deviceIsIOS4) {
      ...
      // 修复滚动过程中可点击的补丁
      if (this.layer.isScrolling && targetElement.fastClickScrollParent) {
  ...
  if (
    event.timeStamp - this.lastClickTime < this.tapDelay &&
    event.timeStamp - this.lastClickTime > -1
  ) {
    event.preventDefault();
  }
...
FastClick.prototype.onTouchEnd = function (event) {
  ...
  if (
    touchEndTime - this.lastClickTime < this.tapDelay &&
    touchEndTime - this.lastClickTime > -1
  ) {
...
FastClick.prototype.onClick = function (event) {
  var permitted;
  this.targetElement = event.target; // here reassign this.targetElement(fix input='file' input.click() 修复input file不能点击的bug)
